    ...
    if (response.statusCode == 200) {
      ...
      if (token != null) {
        ...
        if (response.statusCode == 200) {
          ...
          Employee employee = Employee.buildFromJson(employeeInfo);

          await storage.write(key: AppSecuredKey.userInfoObject, value: jsonEncode(employeeInfo));
          await storage.write(key: AppSecuredKey.token, value: token);
          await storage.write(key: AppSecuredKey.name, value: employee.name);
          await storage.write( key: AppSecuredKey.designation, value: employee.designation);
          await storage.write(key: AppSecuredKey.department, value: employee.department);
          await storage.write(key: AppSecuredKey.company, value: employee.company);
                        ...
                        Navigator.pushReplacement(
                            ...
                            MaterialPageRoute(
                                builder: (context) => HomeScreen(
                                  employee: Employee.buildFromJson(employeeInfo),
                                )));
                      },
                      child: Text("Ok"))
                ],
